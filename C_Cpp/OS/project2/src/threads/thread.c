               ...
               thread_func *function, void *aux)
{
  ...
  list_push_back(&t->parent_thread->child_list, &tc->elem);
...
static void init_child_struct(struct thread_child *thread_child, int process_id){
  ...
  thread_child->id = process_id;
  thread_child->t_file = NULL;
}
...
thread_exit (void)
{
  ...
  /* close child thread's file and free child thread struct if any. */
  e = list_begin (&t->child_list); 
  while (e != list_end (&t->child_list))
  {
    struct thread_child *tc = list_entry (e, struct thread_child, elem);
    e = list_next (e);
    if (tc->t_file) {
      file_close (tc->t_file);
    }
    free(tc);
  }

  /* close current thread's open files and free the struct. */
...
static struct thread_child* 
find_child_with_tid_locked (struct thread *t, tid_t child_tid) {
  struct list_elem *e;
  for (e = list_begin (&t->child_list); e != list_end (&t->child_list);
       e = list_next (e))
  {
    struct thread_child *t = list_entry (e, struct thread_child, elem);
    if(t->id == child_tid) return t;
  }
  return NULL;
}

struct thread_child* 
find_child_with_tid (struct thread *t, tid_t child_tid) {
...
struct thread_child* 
thread_set_child_exit_status (struct thread *t, tid_t child_tid, int status) {
  lock_acquire(&t->child_list_lock);
  struct thread_child* tc = find_child_with_tid_locked(t, child_tid);
  tc->status = status;
  lock_release(&t->child_list_lock);
