...
process_execute (const char *file_name) 
{
  ...
  struct process_arg args = {&status, &sema, &save_ptr, cmd_name, fn_copy};
...
start_process (void *arg)
{
  ...
  struct process_arg *args = (struct process_arg*) arg;
...
load (const char *file_name, char **save_ptr,  void (**eip) (void), void **esp) 
{
  ...
  lock_acquire(&fileSystem);
  unsigned ret = file_read (file, &ehdr, sizeof ehdr);
  lock_release(&fileSystem);
  ...
  *eip = (void (*) (void)) ehdr.e_entry;
  ...
  if (success) {
    find_child_with_tid (thread_current ()->parent_thread, thread_current()->tid)->t_file = file;
  } else {
    lock_acquire (&fileSystem);
    file_close (file);
    lock_release (&fileSystem);
...
/* Create a minimal stack by mapping a zeroed page at the top of
   user virtual memory. */
static bool
setup_stack (const char *res, char **save_ptr, void **esp) 
{
  uint8_t *kpage;
  bool success = false;
  ...
  if (kpage != NULL) 
    {
      ...
      if (success) {
        *ep = PHYS_BASE;
        ...
        /* Word align */
        char *begin = *ep;
        for(; ((size_t)*ep) % sizeof(void *); *(--(*ep))=0);
        ...
        *ep -= (argc + 1) * sizeof(int*);


        /* push argv. */
        *ep -= sizeof(char**);
        *((char**)*ep) = *ep + sizeof(char**);
